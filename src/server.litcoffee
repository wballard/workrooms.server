...
at the moment these configs are just checked in.
        ...
        if config[detail.runtime]
          ...
          console.log "There was no config prepared for extension #{detail.runtime}".yellow
...
Set up a screen sharing session, one peer starts if off similar to a call and
then messages are sent to each side. This is in fact started from a call, you 
don't share a screen without first being call connected to another user.

      socket.on 'sharescreen', (detail) ->
        if detail.outbound
          to = detail.tosessionid
        if detail.inbound
          to = detail.fromsessionid
        tosocket = sockets[to]
        if tosocket
          console.log socket.screenshares
          if _.any(socket.screenshares, (call) -> call.tosessionid is to)
            console.log "already connected screen #{socket.sessionid} to #{tosocket.sessionid}".yellow
          else
            console.log "connecting screen #{socket.sessionid} to #{tosocket.sessionid}".blue
            callid = uuid.v1()
            outboundcall =
              id: uuid.v1()
              outbound: true
            ...
            inboundcall =
              ...
              tosessionid: tosocket.sessionid
              userprofiles: socket.userprofiles
            socket.screenshares.push outboundcall
            socket.signal 'outboundscreen', outboundcall
            tosocket.screenshares.push inboundcall
            tosocket.signal 'inboundscreen', inboundcall
...
the client.
      ...
      socket.on 'hangup', (hangupCall) ->
        ...
        hangupShares =  _.remove(socket.screenshares, (x) -> x?.callid is hangupCall.callid)
        hangupThese = hangupCalls.concat(hangupShares)
        _.forEach hangupThese, (call) ->
...
Presence queries. This uses fulltext as a first pass fuzzy, then filters down.
